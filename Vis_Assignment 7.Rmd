...
boston_nhoods$label <- 
  paste(boston_nhoods$Name, "<br>", 
        boston_nhoods$number_tested, " Total tests by neighborhood") %>% 
...
This interpolation is both helpful in filling in information regarding what is happening inbetween, but also somewhat not completely truthful because I've input dummy data of '1" for neighborhoods that were aggregated by the Boston Public Health Commission, as described in the first map. 
```{r, echo=FALSE}
leaflet(nhood_points) %>%
  addProviderTiles(providers$Stamen.TonerLite) %>%
